...
## M4 参考价与出界判定
- [x] 4.1 `market/reference.py`：可插拔参考价源接口
- [x] 4.2 兜底实现：「价格持续位于界外 ≥ 180 秒」的时间确认
- [x] 4.3 基差相对 EWMA 的突变判定与插针挂起逻辑（`basis_jump_threshold` 0.4%、`pin_timeout` 10min）
- **验收**：用录制的池价序列回放，插针样本不触发重组
...
## M10 监控与记录
- [ ] 10.1 Telegram 告警（建仓、出界、插针挂起、重组、撤出、熔断、心跳失联）
