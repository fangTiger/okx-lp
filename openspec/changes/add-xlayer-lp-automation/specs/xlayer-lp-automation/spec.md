...
### Requirement: 显式停用上市地时段闸门
系统 SHALL 仅在运维显式传入停用开关时跳过上市地交易时段检查，并在启动横幅、日志与判定原因中留下中文警示；默认行为 MUST 保持不变。

#### Scenario: 显式停用后处于上市地交易时段
- **WHEN** 运维传入 `--ignore-sessions` 且当前处于任一上市地交易时段
- **THEN** 系统继续做市，并明确提示时段闸门已停用

#### Scenario: 事件文件不可用
- **WHEN** 上市地时段闸门已停用但事件文件不可用
- **THEN** 系统仍按存在事件处理并撤出

#### Scenario: 财报或外汇保护窗口
- **WHEN** 上市地时段闸门已停用但当前处于财报窗口或外汇周日开盘保护窗口
- **THEN** 系统仍强制撤出

#### Scenario: 未显式停用
- **WHEN** 运维未传入 `--ignore-sessions`
- **THEN** 系统继续按所有上市地时段并集撤出，且启动横幅不显示停用警示
